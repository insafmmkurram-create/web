...
export async function POST(request: NextRequest) {
  try {
    ...
    const { subadminId, email, password } = body
    ...
    if (email) {
      try {
        await auth.updateUser(subadminId, {
          email: email,
    ...
    const userRef = db.collection(usersCollection).doc(subadminId)
    ...
    return NextResponse.json(
      { error: error.message || "Failed to update subadmin" },
      { status: 500 }
    )
  }
}
